...
public class ObraController {
    ...
    @PostMapping
    public ResponseEntity<Obra> criar(@RequestBody ObraInputDTO obraInputDTO) {
        return ResponseEntity.status(HttpStatus.CREATED).body(obraService.criarObra(obraInputDTO));
    }

    @PutMapping("/{idObra}")
    public ResponseEntity<Obra> atualizar(@PathVariable UUID idObra, @RequestBody ObraInputDTO obraInputDTO) {
        return ResponseEntity.status(HttpStatus.OK).body(obraService.atualizarObra(idObra, obraInputDTO));
    }

    @GetMapping("/{idObra}")
    public ResponseEntity<Obra> buscar(@PathVariable UUID idObra) {
    ...
    public ResponseEntity<Obra> excluir(@PathVariable UUID idObra) {
